...
- **Fan-out search** — when an exact FTS5 query returns no results, each extracted keyword is searched independently in parallel; results are merged and composite-scored (BM25 35%, keyword hit breadth 30%, recency 20%, title match 15%) with a project affinity boost for the configured default project; this converts FTS5's AND semantics into effective OR while still ranking multi-keyword hits highest
...
3. **Fan-out search** — FTS5 uses AND semantics by default, so a multi-term query requires every term to appear in a single memory. Auto-recall bypasses this by running parallel searches for each extracted keyword, then merging all results with composite scoring: BM25 rank (35%), keyword hit breadth across queries (30%), recency decay (20%), and title-match bonus (15%), with a project affinity boost for the configured default project. Multi-keyword hits rank highest.
